...
#define PTPD_DBG
#define PTPD_ERR
#define DBGV(...)  { TimeInternal tmpTime; ptpd_get_time(&tmpTime); pd("(d %li %li)", tmpTime.seconds, tmpTime.nanoseconds); pd(__VA_ARGS__); }
#else
#define DBGV(...)
#endif
...
#ifdef PTPD_DBG
#define PTPD_ERR
#define DBG(...)  { TimeInternal tmpTime; ptpd_get_time(&tmpTime); pd("(D %li %li)", tmpTime.seconds, tmpTime.nanoseconds); pd(__VA_ARGS__); }
#else
#define DBG(...)
#endif
...
//
#ifdef PTPD_ERR
#define ERROR(...)  { TimeInternal tmpTime; ptpd_get_time(&tmpTime); pd("(E %li %li)", tmpTime.seconds, tmpTime.nanoseconds); pd(__VA_ARGS__); }
/* #define ERROR(...)  { pd("(E) "); pd(__VA_ARGS__); } */
#else
#define ERROR(...)
#endif
